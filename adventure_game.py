...
def intro():
    ...
    narrator("DM say's : 'Things are not looking pretty, boys. The orc warchief"
             " is \nwreaking havoc on your lines.'\n\n", 1)

    narrator("'Your soldiers are falling one by one with each swing of his axe."
             "\n\n", 1)

    narrator("'Sigurdr, Torag has flung himself into the fray as usual...'\n\n"
             , 1)
...
def the_flank():
    ...
    if "Torag" not in companions:
        narrator("You see Torag throwing orders everywhere as he swings his "
                 "bastard sword.\n\n", 2)
...
def engage_together():
    ...
    global HPSigurdr
    global HPTorag
    playerinput = valid_input_ingame("'Since you have Torag with you, you think"
                                     " your chances of taking him\ndown are "
                                     "slightly higher'\n\n",
                                     "Enter 1. To Attack\n"
                                     "Enter 2. To Go Back\n", ['1','2'])
    if playerinput == '1':
...
def fight_alone():
    ...
    if dieroll >= 18:
        ...
        narrator("'Soldiers are cheering your name and celebrating as the orcs "
                 "lose heart\n and flee !'", 2)
        ...
        if dieroll < 9:
            narrator("'You dodge the axe at the last second and barely manage\n"
...
def init():
    ...
    game()
game()
# couldn't figure out how to use the "scope".
# global variables not reseting when play_again()
#maybe i should define functions for each HP and return ?
